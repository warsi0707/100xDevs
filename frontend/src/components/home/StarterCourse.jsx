...
export default function StarterCourse() {
    ...
    const Course =async()=>{
        try{
            const response = await fetch("https://one00xdevs-cx2s.onrender.com/api/starter",{
                method: "GET"
            })
  ...
  return (
    ...
    <div className='grid sm:grid-cols-2 md:grid-cols-3 gap-5 md:w-[1000px] mx-auto'>
      {loading? <><h1 className='text-3xl font-bold text-black mx-auto text-center my-10'>Loading...</h1></>: ""}
      {course.map((item) =>(
      <div key={item._id} className="card w-72 md:w-80 h-auto mx-auto border-2 border-gray-400 rounded-2xl">
